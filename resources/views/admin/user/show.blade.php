             ...
             <div class="lg:flex">
                <div class="flex flex-col items-start justify-center lg:w-1/3">
                  ...
                  <div class="w-full text-center">
                    ...
                    <span class="text-sm text-zinc-700 dark:text-zinc-400 font-semibold block">{{ $user->email }}</span>
                    <span class=" text-zinc-700 dark:text-zinc-400 font-semibold block">{{ $user->phone_number }}</span>
                  </div>
               ...
               <div class="content lg:w-2/3 md:h-[250px] py-3 overflow-y-auto flex flex-col justify-center sm:justify-start  my-8 lg:my-0 bg-slate-200 dark:bg-slate-600 px-4 rounded-md">
                 <h2 class="text-[#C83B1A] font-bold uppercase text-lg mb-2 text-start sm:text-center">Indirizzi Registrati</h2>
                 @foreach ($user->addresses as $address)
                   <div class="mb-1 text-gray-500 dark:text-gray-400">
                     - <span class="text-[#C83B1A]">indirizzo: {{ $address->road }}</span>
                     <span>città: {{ $address->city }}, {{ $address->zip_code }}</span>
                   </div>
                 @endforeach
                ...
                <div class="my-3 bg-slate-200 dark:bg-slate-600 px-4 py-2 rounded-md">
                 ...
                 <div class="flex flex-col md:flex-row items-start sm:items-center justify-between">
                   <div class=" flex gap-2 mb-2 font-medium text-gray-900 flex-wrap space dark:text-white">
                   ...
                   <div>
                    <form method="POST" action="{{route('admin.user.role', $user->id)}}" class="flex flex-col gap-3 items-center justify-end  rounded ">
                        @csrf
              ...
              <div class="mt-7">
                ...
                <div class="h-[250px] overflow-y-auto">
                  <table class="rounded-lg min-w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @foreach ($user->orders->sortByDesc('order_date') as $order)
