                    ...
                    @foreach ($pizzas as $pizza)
                        <tr class="bg-slate-200 min-w-fit border-b-2 border-gray-100 dark:bg-gray-800 dark:border-gray-700 hover:border-b-2 hover:border-[#C83B1A] dark:hover:border-[#C83B1A]">
                            ...
                            <div class="space-x-1 min-w-fit text-nowrap px-3">
                                <a href="{{ route('admin.pizzas.show', $pizza->id) }}" class="font-medium inline-block text-blue-600 dark:text-blue-500 hover:underline px-2 py-2 border rounded-md hover:bg-blue-500  hover:text-white dark:hover:text-white border-blue-500">Visualizza</a>
                                <a href="{{ route('admin.pizzas.edit', $pizza->id) }}" class="font-medium inline-block text-yellow-600 dark:text-yellow-500 hover:underline px-4 py-2 border rounded-md hover:bg-yellow-500  hover:text-white dark:hover:text-white border-yellow-500">Modifica</a>
                            
                                <form onclick="confirmation(event)"  class="inline-block" action="{{ route('admin.pizzas.destroy', $pizza->id)}}" method="POST">
                                    @method('DELETE')
                                    @csrf
                                    <button class="font-medium text-red-600 dark:text-red-500 hover:underline px-2 py-2 border rounded-md hover:bg-red-500  hover:text-white dark:hover:text-white border-red-500" type="button">Elimina</button>
                                </form>                           
                            </div>
                        </td>
                    </tr>
